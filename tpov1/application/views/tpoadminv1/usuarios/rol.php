        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    <?php echo anchor("tpoadminv1/usuarios/usuarios/alta_rol", "<button class='btn btn-success'><i class=\"fa fa-plus-circle\"></i> Agregar</button></td>"); ?>
                    <div class="pull-right">
                        <button class="btn btn-default"><i class="fa fa-print"></i> Imprimir</button>
                        <button class="btn btn-default"><i class="fa fa-file"></i> Exportar a Excel</button>
                    </div>
                </div><!-- /.box-header -->
                        ...
                        <tbody>
                            <?php
                                for($z = 0; $z < sizeof($roles); $z++)
                                {
                                    ...
                                    echo "<td> <span class='btn-group btn btn-info btn-sm' onclick=\"abrirModal(" . $roles[$z]['id_rol'] . ", '". $roles[$z]['nombre_rol'] . "', '" .  $roles[$z]['active'] . "')\"> <i class='fa fa-search'></i></span></td>";
                                    echo '<td>' . anchor("tpoadminv1/usuarios/usuarios/editar_rol/".$roles[$z]['id_rol'], "<button class='btn btn-warning btn-sm' title='Editar'><i class=\"fa fa-edit\"></i></button></td>");
...
<script type="text/javascript">
    ...
    var eliminarModal = function(id, name){
        ...
        $('#myModalDelete').find('#mensaje_modal').html('¿Desea eliminar la cobertura ' + name+ '?');
    ...
    var abrirModal = function(id, name, active){
        ...
        /*$.ajax({
            url: '/index.php/tpoadminv1/usuarios/campanas_avisos/get_cobertura/'+id,
            data: {action: 'test'},
            dataType:'JSON',
            beforeSend: function () {
                //Loading('Buscando');
            ...
            success: function (response) {
                if(response){
                    $('#myModal').find('#item_1').html(response.nombre_rol);
