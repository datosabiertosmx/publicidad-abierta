...
--
-- Servidor: db771185753.hosting-data.io
-- Tiempo de generación: 11-08-2019 a las 00:07:20
...
(0, '1', 'Login', 'Ingreso al sistema', '2019-09-12 14:30:00');
...
DROP TABLE IF EXISTS `cat_campana_ambito_geo`;
...
DROP TABLE IF EXISTS `cat_campana_coberturas`;
...
DROP TABLE IF EXISTS `cat_campana_estatus`;
...
DROP TABLE IF EXISTS `cat_campana_objetivos`;
...
DROP TABLE IF EXISTS `cat_campana_subtipos`;
...
DROP TABLE IF EXISTS `cat_campana_temas`;
...
DROP TABLE IF EXISTS `cat_campana_tipos`;
...
DROP TABLE IF EXISTS `cat_campana_tiposTO`;
...
DROP TABLE IF EXISTS `cat_ejercicios`;
...
DROP TABLE IF EXISTS `cat_personalidad_juridica`;
...
DROP TABLE IF EXISTS `cat_poblacion_grupo_edad`;
...
DROP TABLE IF EXISTS `cat_poblacion_lugar`;
...
DROP TABLE IF EXISTS `cat_poblacion_nivel`;
...
DROP TABLE IF EXISTS `cat_poblacion_nivel_educativo`;
...
DROP TABLE IF EXISTS `cat_poblacion_sexo`;
...
DROP TABLE IF EXISTS `cat_presupuesto_conceptos`;
...
DROP TABLE IF EXISTS `cat_roles_administracion`;
...
DROP TABLE IF EXISTS `cat_servicios_categorias`;
...
DROP TABLE IF EXISTS `cat_servicios_clasificacion`;
...
DROP TABLE IF EXISTS `cat_servicios_subcategorias`;
...
DROP TABLE IF EXISTS `cat_servicios_unidades`;
...
DROP TABLE IF EXISTS `cat_so_atribucion`;
...
DROP TABLE IF EXISTS `cat_so_estados`;
...
DROP TABLE IF EXISTS `cat_so_ordenes_gobierno`;
...
DROP TABLE IF EXISTS `cat_tiempo_tipos`;
...
DROP TABLE IF EXISTS `cat_tipo_liga`;
...
DROP TABLE IF EXISTS `cat_trimestres`;
...
DROP TABLE IF EXISTS `rel_campana_grupo_edad`;
...
DROP TABLE IF EXISTS `rel_campana_lugar`;
...
DROP TABLE IF EXISTS `rel_campana_maudio`;
...
DROP TABLE IF EXISTS `rel_campana_mimagenes`;
...
DROP TABLE IF EXISTS `rel_campana_mvideo`;
...
DROP TABLE IF EXISTS `rel_campana_nivel`;
...
DROP TABLE IF EXISTS `rel_campana_nivel_educativo`;
...
DROP TABLE IF EXISTS `rel_campana_sexo`;
...
DROP TABLE IF EXISTS `sys_active`;
...
DROP TABLE IF EXISTS `sys_logico`;
...
INSERT INTO `sys_settings` (`id_settings`, `recaptcha`, `clave`, `tipo`, `active`, `url_docs`) VALUES
(1, '6Lftk2oUAAAAAJg1qTK5k7oi1d--cuEkRiiVR8Ih', NULL, NULL, NULL, 'http://pruebasproyectos.com/TPOv2_DGPA/tpoadminv1/');
...
DROP TABLE IF EXISTS `tab_campana_aviso`;
...
DROP TABLE IF EXISTS `tab_convenios_modificatorios`;
...
DROP TABLE IF EXISTS `tab_facturas_desglose`;
...
--
DROP TRIGGER IF EXISTS `FDESGLOSE_INTSERT_TRIGGER`;
DELIMITER $$
...
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `FDESGLOSE_UPDATE_TRIGGER`;
DELIMITER $$
...
DROP TABLE IF EXISTS `tab_fecha_actualizacion_manual`;
...
DROP TABLE IF EXISTS `tab_ordenes_compra`;
...
DROP TABLE IF EXISTS `tab_presupuestos`;
...
DROP TABLE IF EXISTS `tab_presupuestos_desglose`;
...
DROP TABLE IF EXISTS `tab_sujetos_obligados`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vact_ejercicios`  AS  select `cat_ejercicios`.`id_ejercicio` AS `id_ejercicio`,`cat_ejercicios`.`ejercicio` AS `ejercicio`,`cat_ejercicios`.`active` AS `active` from `cat_ejercicios` where (`cat_ejercicios`.`active` = 1) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vgrafica1`  AS  select `a`.`ejercicio` AS `ejercicio`,`a`.`servicio` AS `servicio`,`a`.`campana` AS `campana`,`a`.`partida` AS `partida`,((select sum(`b`.`monto_desglose`) AS `valor2` from ((`vact_facturas` `f` join `vact_facturas_desglose` `b`) join `vact_ejercicios` `c`) where ((`f`.`id_factura` = `b`.`id_factura`) and (`f`.`id_ejercicio` = `c`.`id_ejercicio`) and (`c`.`ejercicio` = `a`.`ejercicio`))) / (select count(0) from `vpregrafica1` `b` where (`b`.`ejercicio` = `a`.`ejercicio`))) AS `ejercido`,`a`.`tipo` AS `tipo`,`a`.`fecha` AS `fecha`,`a`.`proveedor` AS `proveedor`,`a`.`campana_aviso` AS `campana_aviso`,((select sum(`vtab_presupuesto`.`original`) AS `valor1` from `vtab_presupuesto` where (`vtab_presupuesto`.`ejercicio` = `a`.`ejercicio`)) / (select count(0) from `vpregrafica1` `b` where (`b`.`ejercicio` = `a`.`ejercicio`))) AS `presupuesto`,((select sum(`vtab_presupuesto`.`presupuesto`) AS `valor3` from `vtab_presupuesto` where (`vtab_presupuesto`.`ejercicio` = `a`.`ejercicio`)) / (select count(0) from `vpregrafica1` `b` where (`b`.`ejercicio` = `a`.`ejercicio`))) AS `modificacion`,((select count(0) AS `valor1` from `vtab_proveedores` where ((`vtab_proveedores`.`ejercicio` = `a`.`ejercicio`) and (`vtab_proveedores`.`nombre` not in ('Análisis, estudios y métricas','Gastos de propaganda e Imagen','Impresiones','Internet','Medios impresos','Producción de contenidos','Radio','Televisión','Contratos','Órdenes de compra')))) / (select count(0) from `vpregrafica1` `b` where ((`b`.`ejercicio` = `a`.`ejercicio`) and (`b`.`proveedor` not in ('Análisis, estudios y métricas','Gastos de propaganda e Imagen','Impresiones','Internet','Medios impresos','Producción de contenidos','Radio','Televisión','Contratos','Órdenes de compra'))))) AS `proveedores`,((select count(0) AS `valor1` from ((`vact_campana_aviso` `c` join `vact_ejercicios` `b`) join `cat_ejercicios` `e`) where ((`c`.`id_campana_tipo` in (1,2)) and (`c`.`id_ejercicio` = `b`.`id_ejercicio`) and (`b`.`id_ejercicio` = `e`.`id_ejercicio`) and (`e`.`ejercicio` = `a`.`ejercicio`))) / (select count(0) from `vpregrafica1` `b` where (`b`.`ejercicio` = `a`.`ejercicio`))) AS `totalcampanas` from `vpregrafica1` `a` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vlimite_act`  AS  select `vlimite`.`total` AS `limite` from `vlimite` order by `vlimite`.`total` limit 1 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vmonto_campana`  AS  select `b`.`id_campana_aviso` AS `id_campana_aviso`,sum(`b`.`monto_desglose`) AS `monto` from (`tab_facturas` `a` join `tab_facturas_desglose` `b`) where (`a`.`id_factura` = `b`.`id_factura`) group by `b`.`id_campana_aviso` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vmonto_oc`  AS  select `a`.`id_orden_compra` AS `id_orden_compra`,sum(`b`.`monto_desglose`) AS `monto` from (`tab_facturas` `a` join `tab_facturas_desglose` `b`) where ((`a`.`id_orden_compra` > 1) and (`a`.`id_factura` = `b`.`id_factura`)) group by `a`.`id_orden_compra` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `voc_monto_factura`  AS  select `a`.`id_orden_compra` AS `id_orden_compra`,concat('$ ',format(sum(`b`.`monto_desglose`),2)) AS `monto` from (`tab_facturas` `a` join `tab_facturas_desglose` `b`) where (`a`.`id_factura` = `b`.`id_factura`) group by `a`.`id_orden_compra` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `voc_monto_factura_sf`  AS  select `a`.`id_orden_compra` AS `id_orden_compra`,sum(`b`.`monto_desglose`) AS `monto` from (`tab_facturas` `a` join `tab_facturas_desglose` `b`) where (`a`.`id_factura` = `b`.`id_factura`) group by `a`.`id_orden_compra` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vout_campana_lugar`  AS  select `a`.`id_campana_lugar` AS `ID de lugar`,`a`.`id_campana_aviso` AS `ID de campana o aviso institucional`,`a`.`poblacion_lugar` AS `Lugar` from `rel_campana_lugar` `a` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`usr_ejemplo`@`%` SQL SECURITY DEFINER VIEW `vtop10_campanas`  AS  select `a`.`id_campana_aviso` AS `id_campana_aviso`,sum(`a`.`monto_desglose`) AS `sum(a.monto_desglose)` from `vact_facturas_desglose` `a` group by `a`.`id_campana_aviso` order by sum(`a`.`monto_desglose`) desc limit 10 ;
